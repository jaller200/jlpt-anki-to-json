 ...
 */
export const generateKanjiList = async (outputDir, ankiDir) => {
  ...
  (await readNotes(ankiDir + '/n5-kanji.apkg')).forEach(note => kanjiList[note.front] = 5);
  (await readNotes(ankiDir + '/n4-kanji.apkg')).forEach(note => kanjiList[note.front] = 4);
  (await readNotes(ankiDir + '/n3-kanji.apkg')).forEach(note => kanjiList[note.front] = 3);
  (await readNotes(ankiDir + '/n2-kanji.apkg')).forEach(note => kanjiList[note.front] = 2);
  (await readNotes(ankiDir + '/n1-kanji.apkg')).forEach(note => kanjiList[note.front] = 1);

  // Vocab files should have the format `n<level>-vocab.apkg`.
  (await readNotes(ankiDir + '/n5-vocab.apkg')).forEach(note => vocabList[note.front] = 5);
  (await readNotes(ankiDir + '/n4-vocab.apkg')).forEach(note => vocabList[note.front] = 4);
  (await readNotes(ankiDir + '/n3-vocab.apkg')).forEach(note => vocabList[note.front] = 3);
  (await readNotes(ankiDir + '/n2-vocab.apkg')).forEach(note => vocabList[note.front] = 2);
  (await readNotes(ankiDir + '/n1-vocab.apkg')).forEach(note => vocabList[note.front] = 1);
